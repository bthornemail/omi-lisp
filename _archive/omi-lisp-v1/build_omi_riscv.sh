#!/usr/bin/env bash
# build_omi_riscv.sh - Build RISC-V VM with embedded omi-lisp OSI model
# Usage: guix shell --manifest=omi-dev-manifest.scm -- ./build_omi_riscv.sh
...
# Compile C runtime with omi-lisp headers
$CROSS_CC -std=c99 -O2 -static "$ROOT/omi_riscv_vm.c" -o "$INITROOT/init"
...
# Download Alpine base if needed
